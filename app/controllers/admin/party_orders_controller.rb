class Admin::PartyOrdersController < Admin::BaseController
  ...
  def index
    @party_orders = Order.joins(:user).where(status: %i[processing pending_payment]).merge(users_in_party_meeting_plan).order("#{sort_column} #{sort_direction}").page(params[:page]).per(20)
  end
