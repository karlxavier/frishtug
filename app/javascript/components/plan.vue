<template>
  <div class="container">
    <h3
      class="text-uppercase text-center font-family-montserrat-medium font-size-24"
    ...
    <div class="card-deck" style="margin-bottom: 1.7rem;">
      <template v-for="(plan, index) in plans.slice(0, 4)">
        <div class="card rounded0 text-center" v-bind:key="plan.id">
          <img
            class="card-img-top card__plan_image"
            :src="getImgSrc(plan.attributes.name)"
            alt="Card image cap"
          >
          <div class="card-body card__plan_body">
            <h4 class="card-title">{{ plan.attributes.name }}</h4>
            <p class="card-text plan--description">
              <span v-html="plan.attributes.short_description"></span>
            </p>
            <p class="card-text" v-show="plan.attributes.shipping === 'free'">FREE DELIVERY!</p>
          ...
          <template v-if="index === 3">
            <div class="card-body card__plan_body">
              ...
              <p class="card-text" v-show="plans[4].attributes.shipping === 'free'">FREE DELIVERY!</p>
              <p class="card-text" v-show="plans[4].attributes.shipping === 'paid'">
                <span v-html="plans[4].attributes.shipping_note"></span>
              </p>
              <p class="card-text text-upcase">Note:</p>
              <small class="card-text text-left card__plan_note_text">Just a placeholder text</small>
            </div>
            <div class="card-footer card__plan_footer bg-transparent">
              <a
                href="javascript:void(0)"
